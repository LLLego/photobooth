...
export async function renderThemePicker(mount) {
  ...
  for (const id of themeList) {
    ...
    swatch.className = 'w-16 h-16 rounded-3xl border-2 border-warmth-200';
    swatch.style.background = t.previewColor || t.palette?.background || '#FFFFFF';
    const label = document.createElement('span');
    label.className = 'text-xs text-warmth-700 leading-tight';
...
function applyActiveStates() {
  ...
  cards.forEach((c) => {
    ...
    c.querySelector('span').classList.toggle('ring-4', isActive);
    c.querySelector('span').classList.toggle('ring-warmth-900', isActive);
  });
